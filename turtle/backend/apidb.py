...
def createTasks():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS completed
                ...
                author text not null, acceptor text not null, FOREIGN KEY (author) REFERENCES users(username),
                ''')
    conn.commit()
    conn.close()
...
def getAllTasks():
    ...
    for task in tasks:
        datatosend.append({'id': task[0], 'author': task[1], 'title': task[2],
                          'description': task[3], 'price': task[4], 'status': task[5]})
    conn.commit()
    conn.close()
    return datatosend
...
def addTask(author, title, description, price):
    conn = sqlite3.connect('./database.db')
    c = conn.cursor()
    c.execute('''INSERT INTO tasks (author, title, description, price, status) VALUES (?, ?, ?, ?, ?)''',
              (author, title, description, price, 'Posted'))
    conn.commit()
    conn.close()
